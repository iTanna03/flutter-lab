...
class RenderSliverRadial extends RenderSliverMultiBoxAdaptor {
  ...
  set visibleItemCount(int value) {
    ...
    _visibleItemCount =
        anchor == RadialMenuAnchor.center ? childManager.childCount : value;
    markNeedsLayout();
  }
  ...
  void performLayout() {
    ...
    final childConstraints =
        BoxConstraints.tight(Size(childDimension, childDimension));
    ...
    while (
        angle - (angleArcPerChild / 2) < endAngularOffset && index < length) {
      if (!currentChild.hasSize) {
        currentChild.layout(childConstraints, parentUsesSize: true);
      } else if (child == null) {
        child = insertAndLayoutChild(childConstraints,
            after: currentChild, parentUsesSize: true);
        if (child == null) {
          geometry = SliverGeometry.zero;
          childManager.didFinishLayout();
          return;
      ...
      final childAngle = radialAngle.startAngle -
          angularOffset +
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final center =
        Offset((alignment.x + 1) / 2 * width, (alignment.y + 1) / 2 * height);
